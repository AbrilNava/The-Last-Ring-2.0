...
String    []fondoarchivos = {"1 inicio.jpg","2 Selección.jpg","3 Mundo.jpg","4 Combate.png","5 Game over.jpg"};
...
float     []combateenemigotiempos = {0.5f,1.0f};
float       combateenemigotiempo  = 0.5f;
float     ajuste = 10;
...
AudioPlayer musica_fondo;
{
  ...
  musica_fondo= gestor.loadFile("A Knife in the Dark.mp3");
...
void InicioDraw(){
    ...
    textAlign(CENTER);    
    text("PRESIONA ENTER, ESPACIO PARA CONFIRMAR",width/2,height-10);
}
void InicioKey(){
...
void SeleccionDraw(){
    ...
    else if(seleccion >= 3 && seleccion < 6)     
      avatarselector.display((seleccion-3)*width/3 + 54,height*0.6f - 10,160);
        
    tint(255);  
    for(int i=0;i<3;i++){
      avatarseleccion[i].display(i*width/3 + 64,height*0.25f,140);
      text(avatarseleccion[i].informacion.nombre,i*width/3 + 64 + 70,height*0.25f + 148);
    }
    for(int i=3;i<6;i++){
      avatarseleccion[i].display((i-3)*width/3 + 64,height*0.6f,140);
      text(avatarseleccion[i].informacion.nombre,(i-3)*width/3 + 64 + 70,height*0.6f + 148);
    }
  ...
  textFont(fuenteinstrucciones);
  textAlign(LEFT); 
  text("PRESIONA W,A,S,D PARA SELECCIONAR"+((Personaje)personaje).enemigos,16,height-8);
  textAlign(RIGHT); 
  text("PRESIONA ENTER,ESPACIO PARA CONFIRMAR"+((Personaje)personaje).enemigos,width-16,height-8);
...
void MundoDraw(){
  ...
  textAlign(RIGHT); 
  text("ENEMIGOS: "+((Personaje)personaje).enemigos,width-16,height-16);
  textFont(fuenteinstrucciones);
  textAlign(LEFT); 
  text("RECOLECTA VIDA USANDO [WASD] Y ENTRA AL COMBATE"+((Personaje)personaje).enemigos,16,height-16);
...
void CombateDraw(){
  ...
  textFont(fuenteinstrucciones);
  textAlign(LEFT); 
  text("PRESIONA ENTER-ESPACIO PARA COMBATIR"+((Personaje)personaje).enemigos,16,height-16);
   
  if(enemigos.get(combateenemigo).vida <= 0){
     setestado(MUNDO);
     ((Personaje)personaje).enemigos++;
...
void CombateKeyPressed(){     
    if((key == ' ' || keyCode == ENTER) && !click){
      click = true;
      if(personaje.modataque(tiempoenemigo/combateenemigotiempo * ajuste)){
...
void FinDraw(){
  ...
  textFont(fuente);       
  textAlign(CENTER); 
  text("ENEMIGOS: "+((Personaje)personaje).enemigos,width*0.5,height*0.8);
  
    fill(color(255, 204, 0));  
  textFont(fuenteinstrucciones);
  textAlign(LEFT); 
  text("PRESIONA A PARA CONTINUAR"+((Personaje)personaje).enemigos,16,height-16);
